...
We will take the dataset of seattle weather as an example. it can be downloaded from [the website](https://github.com/russellromney/seattle-weather/blob/master/seattleWeather_1948-2017.csv). This dataset has information on the weather in Seattle for every single day between January 1, 1948 and December 12, 2017. 

We're going to use an RNN to predict whether it will rain in Seattle given information on whether it rained or not for the previous six days. For more details, see [the website](https://www.kaggle.com/code/rtatman/beginner-s-intro-to-rnn-s-in-r).

## 1. Basic setting for RNN

### 1.1 Set up R enviornment
...
We're going to set three parameters for model: 1) The maximum length of the sequence that we're going to look at to try and predict the next item; 2 The batch size, which is the number of distinct sequences to look at at one time during training; 3) The total number of epochs to train for.
...
- Remove any na's. If you accidentally end up with na's in your data, your model will compile and train just fine. But the model will prediction all be NaN (not a number). In this case we do need to do it, as is shown in the diagram above, our data slicing approach ended up adding na's to the dataset.
...
Now, we can get down to preparing it to be fed into our model. First, we're going to need to split our dataset into the input (the six previous days) and the output (the single day we're interested in predicting). Following convention, we call our inputs X and output y.
...
Now we need to split our data in to the testing and training set using the createDataPartition() function from the caret package. 
...
## 3. Building and training RNN

### 3.1 Specifying a RNN model

First, we need to tell Keras what type of model we want to build & initialize, which needs to specify in a sequential model. The "sequence" here refers a sequence of the layers in RNN model architecture, other than the data! (another option is to use the functional API). Let's stick with the sequential model with an input layer, a hidden layer and an output layer.
...
Because model takes raw data as input, we need to specify what the dimensions of our data is. Let's look at the dimensions of the matrix that we'll be passing into our model to help figure out what our input_shape should be.

```{r echo=TRUE}
# dim(X_train)
```

In this case, the 1st dimension is the number of our training samples, the 2nd is the length of input sequence (max_len), and the 3rd refers to the number of features (here only one: whether it rained or not). We get the dimensions of the input array using the dim() function and pass those in as the input_shape.
...
Now that we've specified what our input layer will look like, let's tell Keras about our hidden layer. In this model, we're only going to be using a single hidden layer, but for larger and more complex datasets you might want to have more. For the hidden layer, we use a simple RNN layer. But inside it, each neuron is a model of our input sequence, where the second item depends on the first item, the third item depends on the first and second items, the fourth item depends on the first, second and third items, and so on. Here we stick with six neurons. Since this is a recurrent network layer, there will be six different little models of our six-item input sequences. 
...
At this point, our model has 6 different outputs, one for each of the neurons in the hidden layer. But we want to predict only one output: whether it will rain the next day. So, we need to add an output layer that will take all information and squash it down to a single value. We specify this with the "units" argument. Since we predict whether or not it will rain for one day, the units will be 1. 
...
In this case, we wanted to predict whether it would rain tomorrow or not, so we're probably going to be interested in looking at how accurate our predictions are.
...
This looks pretty good! We could probably have stopped training a couple of epoch earlier, since both accuracy and loss flattened out around 5 epochs. 
